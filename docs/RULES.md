...
- Each Discord role combines named requirement groups. **Role requires** decides how groups combine: **Any requirement** passes when at least one group passes, **All requirements** requires every group to pass. Inside each group, **Group requires** decides how its rules combine the same way. A role with one "Main" group behaves exactly like a simple any/all role. This covers nested logic such as `(NFT A OR NFT B) AND Token C`: role requires all, group one holds the two NFT rules as any, group two holds the token rule.
- Rules across groups can combine EVM and Solana rules. Each rule is evaluated against wallets from its own chain family.
