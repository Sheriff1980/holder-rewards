...
Nested groups such as `(NFT A OR NFT B) AND Token C`, trait checks for non-enumerable EVM collections, and collection-wide Solana NFT rules are not enabled yet. Those collection queries require a replaceable NFT/DAS indexer adapter so projects can choose a free tier, paid service, or self-hosted indexer. Exact EVM ownership follows [ERC-721 `ownerOf`](https://eips.ethereum.org/EIPS/eip-721), multi-token balances follow [ERC-1155 `balanceOf`](https://eips.ethereum.org/EIPS/eip-1155), and exact Solana mint balances use `getTokenAccountsByOwner`.

The Cloudflare deployment runs one rolling member recheck every 15 minutes. This conservative default stays compatible with the Workers Free limit of 50 external subrequests per invocation and rotates through about 96 enrolled members per day. Expired ownership-cache rows are removed by the scheduled job. Large communities should add Cloudflare Queues or another job runner before increasing concurrency. Members can always trigger a fresh, uncached check with `/verify refresh`. See [Cloudflare Workers limits](https://developers.cloudflare.com/workers/platform/limits/).
